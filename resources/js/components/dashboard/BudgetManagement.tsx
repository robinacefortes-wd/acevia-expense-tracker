...
const BudgetManagement: React.FC<BudgetManagementProps> = ({
  ...
  const [editingBudget, setEditingBudget] =
    useState<BudgetWithStats | null>(null);

  const [editForm, setEditForm] = useState<{
    category: string;
    limit: string;
    period: NonNullable<Budget['period']>;
  }>({
    category: '',
    limit: '',
    period: 'month',
  ...
  const budgetList: BudgetWithStats[] = budgets.map(
    budget => ({
      ...budget,
  ...
  const totalBudgeted = budgetList.reduce(
    (sum, b) => sum + b.limit,
    0
  );
        ...
        {budgetList.length === 0 ? (
          ...
          >
            ...
            <p className="theme-text-secondary text-sm text-center max-w-sm">
              Create your first budget using the + button
              below to start tracking your spending limits.
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          onDeleteBudget(budget.category)
                        }
